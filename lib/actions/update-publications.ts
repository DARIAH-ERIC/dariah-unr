...
export async function updatePublicationsAction(
	...
	try {
		...
		const updatedReport = await updateReportComments({
			id: reportId,
			comments: { ...comments, publications: comment },
		});
		...
		if (comment) {
			try {
				await sendEmail({
					...
					text: `A comment on the publication report screen for ${String(updatedReport.reportCampaign.year)} has been submitted by ${updatedReport.country.name}.\n\n${comment}`,
				});
			} catch (error) {
				log.error(error);
			}
