...
export async function updateSoftwareAction(
	...
	try {
		...
		const updatedReport = await updateReportComments({
			id: reportId,
			comments: { ...comments, software: comment },
		});
		...
		if (comment) {
			try {
				await sendEmail({
					...
					text: `A comment on the software report screen for ${String(updatedReport.reportCampaign.year)} has been submitted by ${updatedReport.country.name}.\n\n${comment}`,
				});
			} catch (error) {
				log.error(error);
			}
